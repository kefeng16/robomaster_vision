...
bool sendXYZ(int fd, double *xyz)
{
    ...
    data_ptr[0] = (short)xyz[0]; /*yaw*/

    data_ptr[1] = (short)xyz[1]; /*pitch*/
...
{
    unsigned char send_bytes[10] = {0xA5, 0x5A};

    send_bytes[5] = 0; //(unsigned char)((int16_t)xyz[0] & 0x00FF);

    send_bytes[4] = 0; //(unsigned char)(((int16_t)xyz[0] >> 8) & 0x00FF);

    send_bytes[7] = 0; //(unsigned char)((int16_t)xyz[1] & 0xFF);

    send_bytes[6] = 0; //(unsigned char)(((int16_t)xyz[1] >> 8) & 0xFF);

    Append_CRC16_Check_Sum((uint8_t *)&send_bytes, 10);

    if (10 == write(fd, send_bytes, 10))
        return true;
    return false;
}
